...
export default function AdminObjectDefinitionTable({
	objectDefinitions,
	initSort,
	initFilter,
}: IObjectDefinitionTableProps) {
	...
	useEffect(() => {
		localStorage.setItem('objectDefinitionTableSortModel', JSON.stringify(sortModel))
	}, [sortModel])

	useEffect(() => {
		localStorage.setItem('objectDefinitionTableFilterModel', JSON.stringify(filterModel))
	}, [filterModel])
