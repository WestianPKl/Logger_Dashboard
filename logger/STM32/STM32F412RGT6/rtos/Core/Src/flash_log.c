...
static int flash_log_raw_read(uint32_t physical_index, flash_log_record_t *rec)
{
    ...
    result =  mx25_read(flash_log_addr_from_index(physical_index),
                     (uint8_t *)rec,
                     sizeof(*rec));
...
{   
    ...
    result = mx25_write(flash_log_addr_from_index(physical_index),
                      (const uint8_t *)rec,
                      sizeof(*rec));
    xSemaphoreGive(spiMutex);
    return result;
...
static int flash_log_meta_read(uint16_t addr, flash_log_meta_t *meta)
{
    ...
    result = fm24cl16b_read(addr, (uint8_t *)meta, sizeof(*meta));
...
static int flash_log_meta_write(uint16_t addr, const flash_log_meta_t *meta)
{
    ...
    result = fm24cl16b_write(addr, (const uint8_t *)meta, sizeof(*meta));
...
{   
    ...
    if (xSemaphoreTake(flashLogMutex, portMAX_DELAY) != pdTRUE) {
        return -1;
    }
...
int flash_log_read_oldest(uint32_t index, flash_log_record_t *rec)
{
    ...
    if (flashLogMutex == NULL) return -1;
