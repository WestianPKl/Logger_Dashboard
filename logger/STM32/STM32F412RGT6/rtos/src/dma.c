...
void dma1_uart2_rx_config(uint8_t *dst, uint16_t len)
{
    ...
    DMA1_Stream5->CR |= (0U << DMA_SxCR_MSIZE_Pos) | (0U << DMA_SxCR_PSIZE_Pos);
...
void dma2_uart1_rx_config(uint8_t *dst, uint16_t len)
{
    ...
    DMA2_Stream5->CR |= (0U << DMA_SxCR_DIR_Pos);
    DMA2_Stream5->CR |= DMA_SxCR_MINC | DMA_SxCR_CIRC;
    DMA2_Stream5->CR |= (0U << DMA_SxCR_MSIZE_Pos) | (0U << DMA_SxCR_PSIZE_Pos);

    DMA2_Stream5->FCR = 0;
    DMA2_Stream5->CR |= DMA_SxCR_EN;
}

void dma1_uart2_tx_start(const uint8_t *src, uint16_t len)
{
    ...
    DMA1_Stream6->M0AR = (uint32_t)src;
    ...
    DMA1_Stream6->CR |= (0U << DMA_SxCR_MSIZE_Pos) | (0U << DMA_SxCR_PSIZE_Pos);

    DMA1_Stream6->CR |= DMA_SxCR_TCIE | DMA_SxCR_TEIE | DMA_SxCR_DMEIE;
    DMA1_Stream6->FCR = 0;
...
void dma2_uart1_tx_start(const uint8_t *src, uint16_t len)
{
    ...
    DMA2_Stream7->CR |= (1U << DMA_SxCR_DIR_Pos);
    DMA2_Stream7->CR |= DMA_SxCR_MINC;

    DMA2_Stream7->CR |= (0U << DMA_SxCR_MSIZE_Pos) | (0U << DMA_SxCR_PSIZE_Pos);

    DMA2_Stream7->CR |= DMA_SxCR_TCIE | DMA_SxCR_TEIE | DMA_SxCR_DMEIE;
    DMA2_Stream7->FCR = 0;
    DMA2_Stream7->FCR |= DMA_SxFCR_FEIE;
