                     ...
                     const measurement_sht40_t *sht)
{
    ...
    {
        uint32_t sector_now  = addr / MX25_SECTOR_SIZE;
        uint32_t end_addr    = addr + FLASH_LOG_RECORD_SIZE - 1u;
        uint32_t sector_end  = end_addr / MX25_SECTOR_SIZE;

        if (g_log.count == 0u ||
            sector_now != flash_log_addr_from_index(
                (g_log.write_index == 0u) ? (FLASH_LOG_CAPACITY - 1u)
                                          : (g_log.write_index - 1u)) / MX25_SECTOR_SIZE) {
            if (mx25_sector_erase_4k(sector_now * MX25_SECTOR_SIZE) != 1) {
                return -1;
            }
        }

        if (sector_end != sector_now) {
            if (mx25_sector_erase_4k(sector_end * MX25_SECTOR_SIZE) != 1) {
                return -1;
            }
        }
    }
