...
/**
 * @brief Initialize all onboard peripherals and optional modules required for program operation.
 *
 * This routine performs a full hardware and service bring‑up sequence:
 *   1. PWM / Indicators:
 *      - Configures PWM for RGB LEDs (LED_RED, LED_GREEN, LED_BLUE) and BUZZER.
 *      - Sets PWM wrap to 255 (8-bit resolution) for the LED channels.
 *      - Initializes duty cycles to 0 (off) for LEDs and buzzer.
 *      - Momentarily sets the RGB LED to white (255,255,255) as a startup indicator,
 *        later changed to solid green (0,255,0) upon successful initialization.
 *
 *   2. I2C Bus:
 *      - Initializes the default I2C controller at 400 kHz.
 *      - Assigns I2C function to SDA/SCL pins and enables their pull-ups.
 *      - Declares the I2C pins for board/PIO metadata (bi_decl).
 *
 *   3. Input Switches:
 *      - Initializes SWITCH_1 and SWITCH_2 as GPIO inputs with internal pull-ups enabled.
 *
 *   4. Display:
 *      - Initializes LCD, triggers a backlight "kick" (brightness boost / wake), clears the display,
 *        and prints a "Starting..." status message.
 *
 *   5. Configuration-Dependent Modules:
 *      - Reads persistent configuration via config_get():
 *          * logging_enabled flag cached locally.
 *          * If a sensor type (sht) is configured (values 30, 40, or other), instantiates a BME280
 *            in forced mode (current branches are functionally identical; could be simplified).
 *          * If Wi-Fi is enabled (wifi_enabled == 1), allocates a TCP communication object.
 *          * If real-time clock is enabled (clock_enabled == 1), initializes the PCF8563T RTC over I2C.
 *
 *   6. Final State:
 *      - Sets RGB LED to green to signal successful completion.
 *
 * Memory Management / Ownership:
 *   - Dynamically allocates BME280 and TCP instances with new; ownership is not transferred here.
 *     Ensure matching deletion or migration to smart pointers to avoid leaks if this function
 *     can be called more than once.
 *
 * Configuration Dependencies:
 *   - Relies on config_get() returning a stable reference/struct during execution.
 *
 * Side Effects:
 *   - Modifies global / static hardware state (GPIO, I2C, PWM, LCD, RTC).
 *   - Writes to the display and LED indicators.
 *
 * Threading:
 *   - Assumes single-threaded initialization context; not inherently re-entrant.
 *
 * Error Handling:
 *   - No explicit error checks for hardware init failures or allocation failures.
 *     Consider adding return status or logging for robustness.
 *
 * Potential Improvements:
 *   - Collapse redundant sht sensor selection branches.
 *   - Replace raw new with std::unique_ptr (if STL / heap policy permits).
 *   - Add guard to prevent double initialization.
 *   - Introduce diagnostics/logging for each subsystem init.
 *
 * @note Must be called before any component relying on initialized peripherals.
 * @warning Repeated calls without cleanup may leak dynamically allocated objects.
 */
